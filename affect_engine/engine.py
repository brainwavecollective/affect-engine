...
from .anchor import OLLAMA_AVAILABLE, OllamaAnchor
...
class AffectEngine:
    ...
    def __init__(self, config: AffectConfig):
        ...
        self.logger = logging.getLogger("affect_engine")
        self.logger.setLevel(logging.DEBUG if self.config.debug else logging.INFO)

        # Components (loaded in start())
        self.extractor: Optional[VIBEExtractor] = None
        self.amplifier: Optional[CinematicAmplifier] = None
        self.anchor: Optional[OllamaAnchor] = None
        self.blender: Optional[VIBEBlender] = None
    ...
    async def start(self) -> None:
        self.logger.info("Starting AffectEngine...")
        ...
        self.blender = VIBEBlender(self.config)

        # ---- FULL PIPELINE WARMUP (direct, no executor) ----
        self._extract_sentence_vibes_sync(
        ...
        if self.config.enable_anchor:
            ...
            self.anchor = OllamaAnchor(model=self.config.anchor_model_name)
        ...
        self.logger.info("AffectEngine started")

    async def stop(self) -> None:
        self.logger.info("Stopping AffectEngine...")
        ...
        self.logger.info("AffectEngine stopped")
    ...
    async def process_text(
        ...
        if not self.blender or not self.extractor or not self.amplifier:
            raise RuntimeError("AffectEngine not started. Call `await engine.start()` first.")

        t0 = time.perf_counter()

        text = (text or "").strip()
        if not text:
            return {"vibe": self.blender.current.tolist()}

        self._transcript_count += 1
        self._context_buffer.extend(text.split())

        sentence_vibes, sentences = self._extract_sentence_vibes_sync(text)

        if not sentence_vibes:
            return {
                "vibe": self.blender.current.tolist(),
        ...
        dominant = max(sentence_vibes, key=_dominance_score)
        mean_vibe = np.mean(sentence_vibes, axis=0)
        final_burst = np.clip(0.85 * dominant + 0.15 * mean_vibe, 0.0, 1.0)

        self.blender.apply_burst(final_burst, influence=influence)
    ...
    def _extract_sentence_vibes_sync(self, text: str):
        sentences = nltk.sent_tokenize(text)
        sentence_vibes: List[np.ndarray] = []
    ...
    async def _baseline_worker(self) -> None:
        async with self._baseline_lock:
            ...
            if baseline:
                self.blender.apply_baseline(baseline)
    ...
    def _has_settled(self, threshold: float = 0.02) -> bool:
        diff = np.abs(self.blender.current - self.blender.baseline)
    ...
    def _log_telemetry(
        ...
        self.logger.debug("BASELINE: %s", self.blender.baseline.tolist())
        self.logger.debug("--------------------------------------------------")
